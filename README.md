...
| 2. Knowledge graph extraction | `scripts/`, `Inbox/`, `obisidian-knowledge-graph/` | **Partially built** - the four ingestion scripts work and raw material has landed in `Inbox/`; the transcript processing and vault connectivity agents are still spec-only briefs, and no processed notes or vault graph exist yet |
...
| 4. Documentation automation | `claude-md-memory-workflow/` | **Partially built** - an initial `CLAUDE.md` now exists at the repo root, but the scheduled refresh workflow is still a reference copy, not installed under `.github/workflows/`. A separate weekly dependency-refresh action *is* installed |
